...
See the **``master``** branch for the present (pre-release) master development branch, which contains a reference implementation of GlaDOS (albeit, a still highly incomplete one) that is configured for the Gladys persona. Gladys is a persona exhibited by the original GPT-3/davinci engine (which you might call "davinci classic"). Note that you must now use the name ``davinci:2020-03-05`` to access the original version of davinci.
