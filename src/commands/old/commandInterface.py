...
class CommandInterface:		# Singleton class for the command interface subsystem.
	...
	"""
		...
		#|----------------------------------------------------------------------
		#|	CommandInterface._commandInterface		[private class data member]
		#|
		...
		_commandInterface = None	# Instance not yet created at module load time.
	...
	def __new__(thisClass, *args, **kwargs):
		...
		return inst
	#__/ End singleton instance creation method for class CommandInterface.
	
	
	def _init(self):
	
		"""
			commandInterface._init()				   [private instance method]
				...
				instance of CommandInterface on the first call to the 
				CommandInterface class.  Please note that subsequent calls 
				to CommandInterface() do NOT invoke this initializer method.
