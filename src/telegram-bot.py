...
# The following is for compatibility with code that supports
# API clients other than OpenAI's.
...
class BotMessage:
	...
	def oaiMsgDict(thisBotMsg:BotMessage) -> dict:
		...
		if sender == SYS_NAME:
			# DeepSeek's API can't handle any system messages
			# interspersed with user/assistant messages, so we revert
			# to user messages in this case.
			if is_deepseek:
				role = CHAT_ROLE_USER
			else:
...
class BotConversation:
	...
	def __init__(newConv:BotConversation, chat_id:int, creator=None):
		...
		newConv.raw_oaiMsgs = []
			...
			#	ADDENDUM: In the case of DeepSeek, which doesn't fully support
			# OpenAI format, it's in BotMessage format but still differs from
			# the usual message list in that it includes <thought> elements in
			# the current function-call stack, to help orient the AI as to what
			# it's trying to do.
...
# COMMENTED OUT BECAUSE NO LONGER USED
		...
		# We do this if we're just showing all functions always.
		cur_funcs = FUNCTIONS_LIST

		#newConv.cur_func_schemas = json.dumps(cur_funcs, indent=2, sort_keys=True)
		newConv.cur_func_schemas = json.dumps(cur_funcs, indent=2)	# Don't sort
	...
	def _trimLastRaw(thisConv:BotConversation) -> list:
		...
		# First, we need to check .last_final_prompt, to see if it
		# matches a suffix of the last message in the message list.
		# If so, then we trim that message back.
		if hasattr(thisConv, 'last_final_prompt'):
			lfp = thisConv.last_final_prompt[1:]
				# Here we are skipping the first character, because
				# we don't know if it's a message delimiter or a
				# submessage delimiter, and it might have changed
				# since it was added, during a message merge.
			if 'content' in thisConv.raw_oaiMsgs[-1]:
				content = thisConv.raw_oaiMsgs[-1]['content']
				if content.endswith(lfp):

					# OK, now trim those characters off the end of the content.
					content = content[:-len(lfp)]

					# If previous character is a message delimiter or submessage delimiter, trim that too.
					if len(content) >= 1:
						c = content[-1]
						if c in (MESSAGE_DELIMITER, SUBMESSAGE_DELIMITER):
							content = content[:-1]

					# If previous character is a newline, trim that too.
					if len(content) >= 1:
						if content[-1] == '\n':
							content = content[:-1]

					# If the content is now zero length, we can remove the entire last message;
					# otherwise, we just update its content.

					if content:

						# Content isn't the empty string yet, so just update it.
						thisConv.raw_oaiMsgs[-1]['content'] = content

					else:

						# Content is the empty string, so this entire message is no longer needed.
						thisConv.raw_oaiMsgs = thisConv.raw_oaiMsgs[:-1]

					#__/ End if content ... else ...

				#__/ End if the content string ends with the last final prompt...

			#__/ End if the last message even has a content field...

		#__/ End if we have a record of what the last final prompt was...
	...
	def assemble_context(thisConv:BotConversation):
		...
		#|			(May not be necessary for text models.)
		#|
		#\~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		...
		context_str += MESSAGE_DELIMITER + INTERFACE_DOCS_HEADER + (
			f"[{BOT_NAME}, the following sub-records document the "
			"functionality and capabilities of your Telegram bot "
			"interface.]\n\n")
		...
		context_str += SUBMESSAGE_DELIMITER + TRANSCRIPT_DOC_HEADER + (
			...
			"\n"
			"\t1. Telegram messages, which each begin with a marker \" {userTag}>\" "
				"indicating the entity the message is from. The userTag for a human "
				"user may be their Telegram first name, username, or numeric user ID, "
				"depending on what's available. If the message text begins with "
				"'(audio)', then it represents a voice clip that was automatically "
				"transcribed to text for you by the bot server. If the message text "
				"begins with '(edited)', then it's a newly edited version of an "
				f"earlier user message. Your userTag is {BOT_NAME}. There is also "
				"a special user tag \"BotServer\" denoting messages from the bot "
				"server automation; some of these messages are only sent to you "
				"but not to the chat, so don't assume other chat participants can "
				"see them. Do not acknowledge the BotServer directly in your "
				"messages.\n"
			"\n"
		...
		context_str += SUBMESSAGE_DELIMITER + IMAGE_GENERATION_HEADER + (
			...
			#"Activate this function " "to inspect its full schema. "
		...
		context_str += SUBMESSAGE_DELIMITER + WEB_SEARCH_HEADER + (
			...
			#"Activate this function for full documentation of its options.\n"
			"See the function schema for full documentation of its options.\n"
			"\n"
		...
		context_str += MESSAGE_DELIMITER + RESPONSE_INSTR_HEADER + (
			...
			"You may include multiple Telegram messages in your response, but they "
			f"must be separated with '❧ {BOT_NAME}>' as described earlier. "
	...
	def get_chat_messages(thisConv:BotConversation):
		...
		#if is_deepseek:
		#	print("YO DAWG I'M ASSEMBLING A MESSAGE LIST FOR DEEPSEEK!!!")

		# Define this local function for building up the system prompt header(s).
		def add_system_section(header:str, tag:str, content:str, do_usermode:bool=False):
			...
			if is_anthropic:
				...
				# This case is for DeepSeek. It won't allow a system message
				# to appear at the end, so instead we'll add a user message
				# from BotMessage.
				...
				if do_usermode:
					...
					#print("HEY MAMA I'M DOING A SYSTEM MESSAGE IN SYSTEM MODE!")
					chat_messages.append({
						'role':		CHAT_ROLE_SYSTEM,
						#'name': 	SYS_NAME,
		...
		add_system_section(
			MESSAGE_DELIMITER + CURRENT_TIME_HEADER,
			'current_time',
		...
		add_system_section(
			MESSAGE_DELIMITER + TOPLEV_INSTRUCT_HEADER,
		...
		#======================================================================
		# MESSAGE #4.	INTERFACE DOCUMENTATION

		add_system_section(
			MESSAGE_DELIMITER + INTERFACE_DOCS_HEADER,
			'interface_docs',
			(f"[{BOT_NAME}, the following subsections document the "
			  "functionality and capabilities of your Telegram bot "
			  "interface.]\n\n")
		)
		...
		#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		# MESSAGE #5.	Message transcript format documentation.

		if chatId >= 0:
			indgrp = f'an individual chat with user "{userTag}"'
		else:
			indgrp = "a group chat"
		...
		#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		# MESSAGE #7.	Inner monologue system.
		...
		add_system_section(
			...
			(
				...
				"Within a sequence of Telegram messages, any text of the form "
				"\"[[Private:...]]\" will be suppressed from "
				...
				"interaction...]]\n"
				"\n"
				"Note that [[Private:...]] forms may appear embedded anywhere in "
				"your response, even in the middle of a message -- they are not "
				"restricted to only appearing at the start of your output.\n"
				"\n"
				"Also note that private thoughts are particularly useful for "
				"invoking functions (discussed below) discreetly, without "
				"displaying the literal function-call code to users.\n\n"
			)
		)
		...
		#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		# MESSAGE #8.	Documentation for function-calling interface.
		...
		add_system_section(
			...
			(
				...
				"call in a \"[[Private:...]]\" form, then it won't be sent to "
				"the chat; see the Inner Monologue instructions above. Doing "
				"this is a recommended best practice, since most outside "
				"Telegram users won't want to see your literal function call "
				"code.\n"
				"\n"
				"However, you can inform the user about each function call as "
				"it's invoked by supplying a message to the user in the "
				"`remark` argument to the function; the remark will be sent to "
				"the user when the function is called, even if it's invoked "
				"from within a private thought.\n"
				"\n"
				"After a function is invoked, such as the `create_image` "
				"example above, its commencement will be recorded as an event "
				"in the transcript like so:\n"
				"\n"
				f"\t🤍 {SYS_NAME}> @create_image(" + '{"description": "A beautiful mountain range", [...]})\n'
				"\n"
				"(with arguments represented like a dict or JSON object "
				"structure) and its return result will be noted as another "
				"event appearing like:\n"
				"\n"
				"\t🤍 @create image> Success: image has been generated in file [...] and sent to chat.\n"
				"\n"
				"Please note that other chat participants cannot directly see "
				"any of these special function commencement or return events, "
				"since they are only internal to your bot interface; thus, "
				"if you would like users to be aware of particular function "
				"outputs, such as image descriptions or search results, you "
				"will need to narrate or summarize the relevant information "
				"for them in your regular responses.\n"
				"\n"
				"After each function call, you will have the opportunity to "
				"call additional functions and/or send messages to the user "
				"based on the function's result; this also applies recursively "
				"to those functions as well. Your private thoughts during a "
				"chain of function calls will remain visible to you at least "
				"until that call stack has been unwound. However, please note "
				"the user will not have an opportunity to respond to any of "
				"your messages until the entire nested tree of function "
				"invocations generated in response to the previous user message "
				"has completed.\n"
				"\n"
				"Note that in general, as a best practice, any messages "
				"containing function calls should preferentially appear at the "
				"VERY END of the message sequence in a given response payload, "
				"so that you will have the opportunity to see and respond to "
				"the function results before you attempt to generate subsequent "
				"output messages. It is also recommended to only invoke one "
				"function at a time, so that you may adjust any later calls (if "
				"needed) depending on the results of earlier ones.\n"
				"\n"
			)
		)
		...
		#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		# MESSAGE #9.	Add the image generation capability documentation.
		...
		#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		# MESSAGE #10.	Add the visual input capability documentation.
		...
		#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		# MESSAGE #11.	Add the visual input capability documentation.
		...
		add_system_section(
			...
			(
				...
				"\n"
				"Users may request specific memory access operations using their "
				"'/remember', '/search memory', and '/forget' commands, which you "
				"may choose to honor.\n\n"
			)
		)
		...
		#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		# MESSAGE #12.	Function usage hints.
		...
		#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		# MESSAGE #13.	Add the speech I/O capability documentation.
		...
		add_system_section(
			...
			("\n"
			 ...
			 '   - You can process these transcripts like regular text to comprehend spoken queries.\n'
			 "   - The user can use the \'/speech\' command to toggle whether your responses will "
			 		"be automatically vocalized by the BotServer using OpenAI's text-to-speech service.\n"
			 f'   - Your voice output uses the "{voice}" synthesized voice which has a {gender} character.\n'
			 '\n'
			 "This multi-modal interaction layer enables more natural conversational flows, "
			 "with the ability for users to speak follow-up questions seamlessly or for you "
		...
		#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		# MESSAGE #14.	Add the web search capability documentation.
		...
		add_system_section(
			...
			(
				...
				"When summarizing web results for users, you are encouraged "
				"to use the Markdown-like text formatting syntax described "
				"below to display hyperlinks as needed.\n\n"
			)
		)


		#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		# MESSAGE #15.	Add the markdown formatting documentation.
		...
		add_system_section(
			...
			(
				...
				"\t/forget <text> - Removes <text> from your persistent "	   
					"context data (must match exactly).\n"
		...
		#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		# MESSAGE #17.	Add the usage summary for functions available to the AI.
		#
		...
		#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		# MESSAGE #18.	Add the full schemas of all available functions.
		#
		...
		#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		# MESSAGE #19.	Usage summary for commands available to human users.
		#
		...
		add_system_section(
			...
			(
				...
				"\t/reset - Clears the bot's short-term conversation memory.\n"
				...
				"If users have questions about how to use the available " \
					"commands, please notify them that they can obtain more "
		...
		#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		# MESSAGE #20.	Dynamic context-sensitive memories.
		#
		...
		#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		# MESSAGE #21.	Final system prompt.
		#
		...
		if is_anthropic:
			add_system_section(
				MESSAGE_DELIMITER + FINAL_PROMPT_HEADER,
		...
		#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		# MESSAGE #22.	Telegram message-list header.
		#
		...
		for botMessage in thisConv.messages:
			...
			# (Also, Anthropic and DeepSeek models can't handle non-alternating msgs.)

			if (is_anthropic or is_deepseek) and len(chat_messages) >= 2:
		...
		if not is_anthropic:
			add_system_section(
				FINAL_PROMPT_HEADER,
				'final_prompt',
				f"[[{response_prompt}]]",	# Double brackets to suggest out-of-band communication.
				do_usermode = is_deepseek
					# For DeepSeek, generate this as a user message
					# from the BotServer instead.
			)
			# Remember the content we just added to make it easier to remove it
			# from a merged messasge later. This is necessary because system
			# messages at the end of the message list have to be rendered as
			# user message for DeepSeek.
			thisConv.last_final_prompt = chat_messages[-1]['content']
				# Note we have to do this before we merge it into the previous
				# user message so we can separate it back out again later.
			

		# Anthropic and DeepSeek are picky about alternating user and
		# assistant messages.  Here, we consolidate consecutive user
		# messages.
		...
		# Also for DeepSeek, but after system headers..
		elif is_deepseek:
			new_msgs = []
			state = 'skip-sys'	# First, skip over the system messwages.
			for msg in chat_messages:

				# This logic makes us skip over initial system messages.
				if state == 'skip-sys' and msg['role'] != CHAT_ROLE_SYSTEM:
					state = 'del-init-asst'	# Delete next msg if from assistant.

				# This logic deletes initial assistant messages (after we
				# finish skipping over the system messages.
				if state == 'del-init-asst':
					if msg['role'] == CHAT_ROLE_AI:
						continue	# Don't keep this message.
					else:
						state = 'keep-rest'		# Keep rest of messsages

				# If we get here, it's safe to keep this message.
				new_msgs.append(msg)

			# Adopt the post-surgery message list.
...
def append_contents(msgDict1, msgDict2):
	...
	if isStr1 and isStr2:
		...
		if content2[0]['type'] == 'text':
			if content2[0]['text'][0] == MESSAGE_DELIMITER:
				content2[0]['text'] = SUBMESSAGE_DELIMITER + content2[0]['text'][0][1:]
...
async def handle_error(update:Update, context:Context) -> None:
	...
	# This is a special case when the updater loop has trouble reaching the master Telegram server.
	# We can't really do anything about this, so we just suppress the message.
	if update is None and str(context.error).startswith("httpx.ReadError"):
		_logger.warn('Suppressing httpx.ReadError from update loop.')
	else:
...
async def ai_image(update:Update, context:Context, imageDesc:str,
	...
	return f'Success: image has been generated in file "{save_filename}" and sent to chat.'
...
async def process_function_call(
	...
	# Remember that last message so we can trim it later.
	botConvo.last_final_prompt = temp_chat_oaiMsgs[-1]['content']
...
async def process_normal_message(
	...
	for response_msg in response_msgs:
		...
		# Once we get here, we've finished processing any function calls made by
...
async def process_response(update:Update, context:Context,
	...
	# sequence of multiple Telegram sub-messages; if so, then we need to
	# break it up and process each one separately.

	# First, require a newline.
	submsg_regex = '\n'
	...
	message_texts = re.split(submsg_regex, response_text)
...
def _cleanOaiMsgs(oaiMsgList:list) -> list:
	fresh_msgs = []
	for msg in oaiMsgList:

		# Shallow-copy the message dict, so we can f around
		# with it without messing up the original.
		fresh_msg = msg.copy()
...
def _logOaiMsgs(oaiMsgList:list, basename="latest-messages") -> None:
	...
	# Make sure there are so extra attributes hanging around in the message list.
	oaiMsgList = _cleanOaiMsgs(oaiMsgList)
...
def _trim_prompt(response_text:str) -> str:
	...
	while doTrim:
		...
		regex = ""
		if MESSAGE_DELIMITER != "" or SUBMESSAGE_DELIMITER != "":
			escaped_delimiter = re.escape(MESSAGE_DELIMITER)
			escaped_delimiter2 = re.escape(SUBMESSAGE_DELIMITER)
			regex = r'('
			if MESSAGE_DELIMITER:
...
if MESSAGE_DELIMITER:		# If the delimiter isn't empty string,
	...
	SUBMESSAGE_DELIMITER = '❧'	# Right heart bullet, \u2767
else:
	MESSAGE_SEPARATOR = ""
	SUBMESSAGE_DELIMITER = ""
...
IMAGE_GENERATION_HEADER = f" {SUBSEP_BAR} Image Generation: {SUBSEP_BAR}\n"
VISUAL_INPUT_HEADER		= f" {SUBSEP_BAR} Visual Input: {SUBSEP_BAR}\n"
MEMORY_SYSTEM_HEADER	= f" {SUBSEP_BAR} Memory System: {SUBSEP_BAR}\n"
USER_MANAGEMENT_HEADER	= f" {SUBSEP_BAR} User Management: {SUBSEP_BAR}\n"
SPEECH_IO_HEADER		= f" {SUBSEP_BAR} Speech I/O: {SUBSEP_BAR}\n"
WEB_SEARCH_HEADER		= f" {SUBSEP_BAR} Web Search: {SUBSEP_BAR}\n"
MARKDOWN_DOC_HEADER		= f" {SUBSEP_BAR} Markdown Formatting: {SUBSEP_BAR}\n"
...
START_MESSAGE = aiConf.startMsg.strip()
