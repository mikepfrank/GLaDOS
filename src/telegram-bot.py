...
class Conversation:
	...
	def remove_memory(self, text_to_remove:str) -> bool:
		...
		if len(text_to_remove)==0 or text_to_remove[-1] != '\n':
			text_to_remove += '\n'
		...
		if text_to_remove not in PERSISTENT_DATA:
			sys_err = f"Error: [{text_to_remove.rstrip()}] not found in persistent memory."
			self.add_message(Message(SYS_NAME, sys_err))
			#update.message.reply_text(f"[{SYS_NAME}: {sys_err}]")
...
def process_message(update, context):
	...
	# Need to add some more error handling here for the case where
	# there is not a conversation object in scope yet. (E.g., bot
	# server was restarted but the /start command hasn't been sent yet.)
	...
	conversation.add_message(Message(userFirstname, update.message.text))

	acl = ['Michael', 'August', 'celeste', 'C']
	
	# Check to see if this is an authorized user.
	if userFirstname not in acl:

		_logger.info(f"process_message(): Rejecting query attempt from user {userFirstname}.")
		
		sys_err = f"Error: We're sorry, but access to the DaVinci v3 bot is currently restricted to authorized users due to excessive usage."
		conversation.add_message(Message(SYS_NAME, sys_err))
		user_err = f"[DIAGNOSTIC: We're sorry, but access to the DaVinci v3 bot is currently restricted to authorized users due to excessive usage.]"
		update.message.reply_text(user_err)
		return
