...
class Configuration:    # The GLaDOS server configuration.
        ...
        def __init__(inst, *args, **kwargs):
                ...
                inst._reinit(*args, **kwargs)

                #|----------------------------------------------------------------------
                #|      ._reinit()                  [singleton instance (re)initializer]
                #|
        ...
        def _reinit(inst, confStruct=None, recheckEnv:bool=False, reloadConf:bool=False):
                ...
                if recheckEnv or not hasattr(inst, 'envChecked'):
                ...
                if confStruct == None:          # No config struct provided; we have to load one.
                        ...
                        if reloadConf or inst._lastLoadedConf == None:
                                confStruct = inst.loadConfig()
                        else:
                                confStruct = inst._lastLoadedConf
        ...
        def process(inst):
                ...
                if inst.appList is not None:

                    _logger.debug(f"About to process {len(inst.appList)} apps...")
        ...
        def checkEnvironment(inst):
                ...
                _logger.debug(f"Constructuring config path from '{_BASEDIR}' + '{_CONFIG_FILENAME}'... ")
